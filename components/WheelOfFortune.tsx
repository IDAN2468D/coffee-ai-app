...
import { X, Gift, Sparkles, Copy, Trophy, Coffee, Star } from 'lucide-react';
...
export default function WheelOfFortune() {
    ...
    useEffect(() => {
        ...
        // DEV MODE: Reduced to 1 minute cooldown for testing instead of 24h
        const lastSpin = localStorage.getItem('lastSpinTime');
        const now = new Date().getTime();

        if (!lastSpin || now - parseInt(lastSpin) > 60 * 1000) {
        ...
        <>
            ...
            {!isOpen && (
                <motion.button
                    ...
                    className="fixed bottom-6 right-24 z-40 bg-[#C37D46] text-white p-4 rounded-full shadow-2xl border-4 border-[#FDFCF0] flex items-center justify-center group hover:bg-[#2D1B14] transition-colors"
                >
                    <Gift className="w-6 h-6 group-hover:animate-bounce" />
                    <span className="absolute right-full mr-3 bg-[#2D1B14] text-white px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity shadow-lg pointer-events-none">
                        גלגל המזל
                    </span>
                    {/* Notification Badge */}
